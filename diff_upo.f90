...
program diff_upo
  ...
  ! 2015/Sep/18
  ! this code only consider the time-phase shifting
  ! see diff_upo_phase.f90 for the shift in x,z 
  ! The shift in y is not implemented... 
  use ctes
  use running
  use bcs
  use statistics
  ...
  integer istat(MPI_STATUS_SIZE),ierr,kmax,iph,ist_u,ien_u,idigit_u,idigit,iapply_window_y
  ...
  if (myid.eq.0) then
     ...
     write(*,*) 'diff tool for revisiting to the reference UPO'
  ...
  if (myid.eq.0) then
     ! for checking a file
     fname1 = fname_base(1:index(fname_base,' ')-1)//'.diffu'
  ...
  allocate(xtmp(nmax))
  xtmp=0.d0
  allocate(diff_xp(mTp),time_xp(kmax))
  diff_xp=0.d0;
  allocate(diff_ph(3,mTp))
  diff_ph=0.d0;
  ind=0

  ! ---- read the reference UPO ----
  do iph=1,(ien_u-ist_u+1)
     ...
     if (idigit_u.eq.3) then
        write(ext,'(i3.3)') ist_u + iph -1
        filinp =  upofile(1:index(upofile,' ')-1)//'.'//ext(1:3)
     else
        write(ext4,'(i4.4)') ist_u + iph -1
     ...
     ! sym_shiftx,sym_shiftz is read from initcr()
     !call phase_shift_fou(vor,phi,atan(phasex)/(2*pi)*Lx,atan(phasez)/(2*pi)*Lz)
     !call phase_shift_fou(vor,phi,0.5*Lx,0.0) ! half-phase shift in x
     call phase_shift_fou(vor,phi,sym_shiftx*Lx,sym_shiftz*Lz) ! half-phase shift in z
     ...
     if (iapply_window_y.eq.1) then
        write(*,*) 'iapply_window_y=1'
  ...
  do istep = istart,iend
     ...
     !call phase_shifty(vor,phi,u00,w00,0.5*Ly,fac)
     !call dealiasing_y(vor,phi,u00,w00,dump,dump,dumc,dumc,fac)
     if (iapply_window_y.eq.1) then
        if (myid.eq.0) write(*,*) 'iapply_window_y=1'
        call window_y(vor,phi,u00,w00)
     end if
     ...
     if ((istep-istart+1).le.kmax) then 
        ...
        call throwing(vor,phi,u00,w00,chwk,xp(1,istep),1)
        time_xp(istep)=time
     else
        ...
        diff_xp=0.d0
        diff_ph=0.d0
        ! /* consider the time-phase shift, but only for the multiple of the box period */
        do iTp=0,mTp-1
           ! /* time-integration of L2 norm from t=0 to Tp */
           do ii=0,kmax-1 
              ...
              diff_xp(iTp+1) = diff_xp(iTp+1) + xrnorm 
              !atan(phasex)/(2*pi)
           ...
           diff_xp(iTp+1) = diff_xp(iTp+1)/dfloat(kmax)
        ...
        if(myid.eq.0) then
           !write(*,'(I6,1X,F14.6,32(1X,E14.6) )') &
           write(iout) dfloat(istep),time_xp(ibase+1),xpnorm,(diff_xp(ii),ii=1,mTp)
           write(34) dfloat(istep),time_xp(ibase+1),xpnorm,(diff_ph(1,ii),ii=1,mTp)
           write(35) dfloat(istep),time_xp(ibase+1),xpnorm,(diff_ph(2,ii),ii=1,mTp)
           write(36) dfloat(istep),time_xp(ibase+1),xpnorm,(diff_ph(3,ii),ii=1,mTp)
        end if
