...
subroutine crosst(vor,phi,u00,w00,hv,hg,phiwk,vorwk,dvordy, &
 ...
 if (iadd_damping.eq.1) then
  ...
  !/********************************************************************/
  !/*      da un paso en el tiempo. Runge - Kutta  para terminos       */
  !/*      convectivos euler inverso para la parte viscosa.            */
  !/*                                                                  */
  ...
  !/*     hg: Hg                                                       */
  !/*     hv: Hv                                                       */
  !/*     dvordy: area de trabajo de dimension mx*nxymax               */
  !/*     chwk:   area de trabajo para los chz2y                       */
  !/*                                                                  */
  !/*  output:                                                         */
  !/*     vor: vorticidad segun la direccion y (n+1)                   */
  !/*     phi: laplaciana velocidad segun la direccion y  (n+1)        */
  !/*      hg: contiene  v (velocidad segun y)                         */
  !/*      hv: contiene  dv/dy                                         */
  !/*..................................................................*/
  !/*..................................................................*/
  !/*                                                                  */
  ...
  !/*   in jik form jca                                                */
  !/*   to CFdiff by of                                                */
  !/*   updated jjs Aug/07 for disk                                    */
  !/*   updated A.S. Sep/2011 for shear-periodic HSF                   */
  !/*   RK3 is updated to SMR, A.S. Nov/2011                           */
  !/*   see log_changes later modifications.                           */
  !/*   RK3 integrating shear-convective terms                         */
  !/*          updated, A.S. 14/May/2013                               */
  !/*   LES option is merged with branch, 11/Nov/2014                  */
  !/********************************************************************/
...
!             tmpxzr,tmpx,wk1dr)
  ...
  if (inonNewtonian.eq.1) then
     if (allocated(txy00)) deallocate(txy00)
     if (allocated(tyz00)) deallocate(tyz00)
     if (allocated(wxc))   deallocate(wxc)
     if (allocated(wyc))   deallocate(wyc)
     if (allocated(wzc))   deallocate(wzc)
     if (allocated(vxc))   deallocate(vxc)
     if (allocated(vyc))   deallocate(vyc)
     if (allocated(vzc))   deallocate(vzc)
     if (allocated(uxc))   deallocate(uxc)
     if (allocated(uyc))   deallocate(uyc)
     if (allocated(uzc))   deallocate(uzc)
     ...
     if (allocated(Tzx))   deallocate(Tzx)
...
!/*   input:                                                         */
!/*     phi: Delt(v)   (F-F-Tch)                                     */
!/*     vor: vorticity (F-F-Tch)                                     */
!/*     rhg: velocity along y axis (F-F-phys)                        */
!/*     rhv: dv/dy                 (F-F-phys)                        */
!/*     work: area de trabajo de dimension al menos  max(mgalx,4*my) */
!/*                                                                  */
!/*   output:                                                        */
!/*     rhv: nonlinear term for the phi equation    (F-F-Tch)        */
!/*     rhg: non linear term for vorticity equation (F-F-Tch)        */
...
!/*    compact runge-kutta A.S. : 24/Nov/2011                        */
!/********************************************************************/
  ...
  DO J = JB,JE       !---------------------- start operating by planes
     ...
     if (inonNewtonian.eq.1) then
      ...
      if (iuse_LES.eq.1) then
        ...
        !/* rhvc1=-d^2(T11)/dx^2-d^2(T33)/dz^2-2*d^2(T13)/dxdz+(d^2/dx^2+d^2/dz^2)T22 */
        !/* rhvc2= d(T12)/dx+d(T23)/dz                                                */
        !/* rhgc1= -d^2(T33-T11)/dxdz+(d^2/dz^2-d^2/dx^2)*T13                         */
        !/* rhgc2= d(T12)/dz-d(T23)/dx                                                */
     ...
     ! /****************************************************/
     ! /*         computes u X  omega                      */
     ! /*         u2 = H1 = v.omega_3 - w.omega_2 (F-F-P)  */
     ! /*         u3 = H2 = w.omega_1 - u.omega_3 (F-F-P)  */
     ! /*         u1 = H3 = u.omega_2 - v.omega_1 (F-F-P)  */
     ! /****************************************************/
#ifdef TIMING
     t1=MPI_WTIME()
#endif
     ...
     t1=MPI_WTIME()
#endif
     !/*   u2 = - dH3/dx + dH1/dz,  o1 = dH1/dx + dH3/dz  */
