...
module yderiv
  ...
  real*8, allocatable:: pr1c(:,:),pr1ci(:,:), rh1c(:,:)
  real*8, allocatable:: pr2(:,:), pr2i(:,:), rh2(:,:), lapmat(:,:)
  real*8, allocatable:: pr2c(:,:), pr2ci(:,:), rh2c(:,:), lapmatc(:,:)
  real*8, allocatable:: wk1(:), wk2(:)
  real*8  h
  integer N1,N2, N3, my

  real*8, allocatable:: x(:),dxidx(:)
...
subroutine prederiv1
  ...
  allocate ( pr1(-N:N,my), rh1(-N:N,my),pr1c(-N:N,2:my-1),pr1ci(-N:N,2:my-1),rh1c(-N:N,2:my-1) )
  pr1 = 0d0
  rh1 = 0d0
  pr1c= 0d0
  rh1c= 0d0
...
subroutine prederiv2
  ...
  allocate ( pr2c(-N:N,2:my-1), pr2ci(-N:N,2:my-1), rh2c(-N:N,2:my-1), lapmatc(-N:N,2:my-1)  )
  pr2 = 0d0
  rh2 = 0d0
  pr2c = 0d0
  rh2c = 0d0
  ...
  do j=1,N
     rh2c( j,2:my-1) =  vec(N+1+j)
     rh2c(-j,2:my-1) =  vec(N+1+j)
     pr2c( j,2:my-1) =  vec(j)
     pr2c(-j,2:my-1) =  vec(j)
  enddo
  rh2c(0,2:my-1) = vec(N+1)
  pr2c(0,:)      = 1d0
...
!                             jj/may/05
!                             ym/dec/07 for non-uniform grid
!
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

subroutine derivadas(n,dxi,y,dxidy)
  use yderiv
  implicit none
  integer n,j
  real*8  dxi
...
! ---  non-uniform grid and metric -----------
  allocate (x(my), dxidx(my))
  do j=1,my
    x(j)     = dble(y(j))
    dxidx(j) = dble(dxidy(j))
  enddo
...
!***************************************************************
!***************************************************************
! Subroutines for non-uniform grid
!***************************************************************
!***************************************************************
...
! prederiv2nu:
!
! Computes the coefficients for the cfd for second derivatives 
! on a non-uniform grid.
!
!***************************************************************
subroutine prederiv2nu
  use yderiv
  implicit none
  ...
  N2 = 2
  allocate ( pr2(-N2:N2,my), pr2i(-N2:N2,my), rh2(-N2:N2,my), lapmat(-N2:N2,my)  )
!  allocate ( pr2c(-N2:N2,2:my-1), pr2ci(-N2:N2,2:my-1), rh2c(-N2:N2,2:my-1), lapmatc(-N2:N2,2:my-1)  )
  pr2 = 0d0
  rh2 = 0d0
!  pr2c = 0d0
!  rh2c = 0d0
...
!------------------------------------
  ...
  do j=3,my-2
     ...
     do k=3,8
        c(k+1,1)=k*(k-1)*(x(j-2)-x(j))**(k-2)
        c(k+1,2)=k*(k-1)*(x(j-1)-x(j))**(k-2)
        c(k+1,3)=k*(k-1)*(x(j+1)-x(j))**(k-2)
        c(k+1,4)=k*(k-1)*(x(j+2)-x(j))**(k-2)
        c(k+1,5)=-(x(j-2)-x(j))**k
        c(k+1,6)=-(x(j-1)-x(j))**k
        c(k+1,7)=0d0
        c(k+1,8)=-(x(j+1)-x(j))**k
...
!!!! this scheme is singular for uniform mesh ??? 
  ...
  c(5,1)=1.2d1*(x(j+1)-x(j))**2
  c(5,2)=1.2d1*(x(j+2)-x(j))**2
  c(5,3)=0d0
  c(5,4)=-(x(j+1)-x(j))**4
  c(5,5)=-(x(j+2)-x(j))**4
  b(5)=0d0
...
!-----------------------------------------------------------------------
  ...
  !!!!!!!!!!!!!!!!!!  compute explicit first derivative for end-pointis
  allocate(endder(0:N2), mat(0:N2,0:N2))
  mat(0,:) = 1d0
  endder   = 0d0
  endder(1)= 1d0
  ...
  call gaussj(mat,N2+1,N2+1,endder,1,1)
...
subroutine derivycnu(u,du,iord,m)
  use yderiv
  ...
  if (iord.eq.1) then    !!!!!  first derivative
     ...
     do i=1,m
        ...
        call banbks(pr2i,my,wk1)
        du(i,:)=wk1
     enddo

  endif

end subroutine derivycnu




subroutine lapcdynu(u,k2,p,m,bc1,bc2,flag)
  use yderiv
  ...
  do i=1,m
     ...
     call banbks(lapmat,my,wk1)
     p(i,:) = wk1

  enddo

end subroutine lapcdynu
