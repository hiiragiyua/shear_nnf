...
subroutine prederiv1
  ...
  allocate ( pr1(-N:N), rh1(-N:N) )
  allocate ( a1(my),b1(my),c1(my),d1(my))
  allocate ( e1(my),f1(my),g1(my),ce1(my),cf1(my),cg1(my) )
  !write(*,*) '  prederiv1: allocated'
  pr1 = 0d0; rh1 = 0d0
  a1  = 0d0; b1  = 0d0; c1  = 0d0; d1 = 0d0; 
...
subroutine prederiv2
  ...
  allocate ( a2(my),b2(my),c2(my),d2(my) )
...
subroutine visc3d(u,visc,ddu,shp,shm,m,rk,scal,what)
  use yderiv
  implicit none

  integer m
  real*8 u(m,my),visc(m,my),ddu(m,my)
  real*8 rk,scal
  complex*16 shp,shm
  character*1 what

! 2nd derivative in 'y':
  call derivyc(u,ddu,shp,shm,2,m)
...
subroutine derivyc(u,du,shp,shm,iord,m)
  use yderiv

  implicit none
  integer i,j,m,jm3,iord
  ...
  if (iord.eq.1) then    !!!!!  first derivative
     ...
     if (m.eq.1) then
        ...
        dh=d1;eh=e1;fh=f1;gh=g1;
        call modChol_addshear(a1,b1,c1,dh,eh,fh,gh,ceh,cfh,cgh,pr1,my,shp)
!        write(*,*) 'c1',c1
!        write(*,*) 'd1',dh
!        write(*,*) 'e1',eh
!        write(*,*) 'f1',fh
!        write(*,*) 'g1',gh
        ...
        call solve7cyclic(a1,b1,c1,dh,eh,fh,gh,ceh,cfh,cgh,wk1,du,wk2,my)
     endif
     ...
     if (m.eq.1) then
        ...
        dh=d2;eh=e2;fh=f2;gh=g2;
        call modChol_addshear(a2,b2,c2,dh,eh,fh,gh,ceh,cfh,cgh,pr2,my,shp)
        call solve7cyclic(a2,b2,c2,dh,eh,fh,gh,ceh,cfh,cgh,wk1,du,wk2,my)
     endif
  endif
...
!***********************************************************************
subroutine modChol_addshear(La,Lb,Lc,Dd,Le,Lf,Lg,cLe,cLf,cLg,a,N,shp)
  integer N
  real*8  La(N),Lb(N),Lc(N),Dd(N)
  !complex*16 La(N),Lb(N),Lc(N),Dd(N)
  complex*16 Le(N),Lf(N),Lg(N),cLe(N),cLf(N),cLg(N)
