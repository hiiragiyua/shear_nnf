...
!             tmpxzr,tmpx,wk1dr)
        ...
        if (explicit.eq.1) then
           ...
           if (myid.eq.0) then
              ...
              shp=dcmplx(1.d0,0.d0); shm=dcmplx(1.d0,0.d0)
              call lapcdy(u00wk,dtr1,u00,shp,shm,1) ! real*8
              shp=dcmplx(1.d0,0.d0); shm=dcmplx(1.d0,0.d0)
              call lapcdy(w00wk,dtr1,w00,shp,shm,1) ! real*8
...
subroutine hvhg(phic,ome2c,u00,w00,rhvc,rhgc, &
  ...
  DO J = JB,JE       !---------------------- start operating by planes
     ...
     u1c(0,0) =  u00(j)  ! does not include mean shear s*y
     u3c(0,0) =  w00(j)
  ...
  if ((rkstep.eq.1).and.(icfl.eq.1)) then
     ...
     if (ifix_dt.eq.1) then
        ...
        if ((CFLv.gt.CFL).and.(explicit.eq.1)) then
           dt = 1.d0/cflvv
